...
# 🧠 LOOP & DUPLICATE PROTECTION TRACKER
PROCESSED_MESSAGES = set()
...
# ------------------------------------------
# 🎯 STEP 2: KEYWORD DETECTOR, WATERMARKER & DUPLICATE DELETER
# ------------------------------------------
@client.on(events.NewMessage(chats=MY_TARGET_CHANNEL))
async def auto_logo_handler(event):
    if not event.message:
    ...
    # 🛑 1. CRITICAL DUPLICATE DELETER FEATURE
    # Agar ye message ID already processed hai ya hamari footer link andhadhundh repeat ho rahi hai
    if event.message.id in PROCESSED_MESSAGES:
        try:
            await client.delete_messages(event.chat_id, event.message.id)
            print(f"🗑️ [EXTRA FEATURE] Duplicate message detected (ID: {event.message.id}) and instantly deleted from channel!")
        except Exception as e:
            print(f"❌ Duplicate delete error: {e}")
        return

    original_text = event.message.message or ""
    
    # 🛑 2. TEXT DUPLICATE PROTECTION
    if "https://t.me/sixtystore_loot" in original_text:
        # Loop safety tracker mein daal do taaki system track rakhe
        PROCESSED_MESSAGES.add(event.message.id)
        return

    # Pehle hi step par incoming message ID ko block list mein daal do
    PROCESSED_MESSAGES.add(event.message.id)

    # Secret keyword tracker check
    is_bhrams_post = SECRET_KEYWORD in original_text
    clean_text = original_text.replace(SECRET_KEYWORD, "").strip()
    ...
    # 📸 CASE B: AGAR PHOTO WALA MESSAGE HAI
    if not is_bhrams_post:
        print("⏩ Kisi aur channel ki post hai, image edit SKIP. Sirf text link add होगी।")
        try:
            final_text = clean_text + MY_BRAND_FOOTER
            await client.edit_message(event.chat_id, event.message.id, final_text)
            print("📝 [SUCCESS] Normal post par link add ho gayi bina image chhede!\n")
    ...
    # 🎯 TARGET VERIFIED: Logo processing chalu karo
    print("🎯 SECRET KEYWORD FOUND! BhramsBots1 verified. 2-Logo editing chalu...")
    try:
        photo_path = await event.message.download_media()
        base_img = Image.open(photo_path).convert("RGBA")
        ...
        # 🗑️ Purana duplicate post delete karo jo converter bot ne dala tha
        await client.delete_messages(event.chat_id, event.message.id)
        print("🗑️ Old unbranded post deleted successfully.")

        # Naya customized post bhejdo clean text aur footer ke sath
        final_text = clean_text + MY_BRAND_FOOTER
        sent_msg = await client.send_file(MY_TARGET_CHANNEL, "edited_photo.jpg", caption=final_text)
        
        # 🔐 CRITICAL STEP: Naye message ki ID ko instantly system tracker mein block karo
        PROCESSED_MESSAGES.add(sent_msg.id)
        print(f"💥 [SUCCESS] Clean 2-Logo post successfully re-uploaded! Protected ID: {sent_msg.id}\n")
